-- Recreate credit RPCs with safe handling of missing tables and proper move/credit deduction.
...
begin
  select to_regclass('public.credit_refill_state') is not null into has_moves;

  if has_moves then
    select coalesce(moves_left, 0) into v_moves
    from public.credit_refill_state
    where player_id = p_player_id
    for update;
  end if;

  if has_moves and v_moves > 0 then
