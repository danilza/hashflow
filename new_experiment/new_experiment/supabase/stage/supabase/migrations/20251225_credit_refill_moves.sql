-- Ensure credit_refill_state has moves_left column used by charge_run_attempt.
ALTER TABLE public.credit_refill_state
  ADD COLUMN IF NOT EXISTS moves_left int DEFAULT 0;

UPDATE public.credit_refill_state
SET moves_left = COALESCE(moves_left, 0);

-- Recreate helper to avoid stale dependency cache.
...
-- Recreate charge_run_attempt to reflect column existence.
